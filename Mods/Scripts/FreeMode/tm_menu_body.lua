...
label TMMenuBody(human)
	...
	+ "Neck.. | " .. necksize [gold]
		+ "Neck >"
			necksize = BodyEditUp(human, "Neck size", necksize, BodyStepC, 2)
			Return()
		+ "< Neck"
			necksize = BodyEditDown(human, "Neck size", necksize, BodyStepC, -1)
			Return()
		+ "RESET | " .. necksize [gold]
	...
	+ "Arms.. | T" .. forearmsize .. " | C" .. uperarmsize [gold]
		+ "Forearms >"
			forearmsize = BodyEditUp(human, "Forearms size", forearmsize, BodyStepC, 2)
			Return()
		+ "< Forearms"
			forearmsize = BodyEditDown(human, "Forearms size", forearmsize, BodyStepC, -0.5)
			Return()
		+ "RESET Forearms | " .. forearmsize [gold]
			forearmsize = BodyEditSet(human, "Forearms size", 0)
			Return()
		+ "Upper arms >"
			uperarmsize = BodyEditUp(human, "Upper arms size", uperarmsize, BodyStepC, 2)
			Return()
		+ "< Upper arms"
			uperarmsize = BodyEditDown(human, "Upper arms size", uperarmsize, BodyStepC, -0.5)
			Return()
	...
	+ "Legs.. | T" .. thighsize .. " | C" .. calfsize [gold]
		+ "Calf >"
			calfsize = BodyEditUp(human, "Calf size", calfsize, BodyStepC, 2)
			Return()
		+ "< Calf"
			calfsize = BodyEditDown(human, "Calf size", calfsize, BodyStepC, -0.5)
			Return()
		+ "RESET Calf | " .. calfsize [gold]
			calfsize = BodyEditSet(human, "Calf size", 0)
			Return()
		+ "Thigh >"
			thighsize = BodyEditUp(human, "Thigh size", thighsize, BodyStepC, 2)
			Return()
		+ "< Thigh"
			thighsize = BodyEditDown(human, "Thigh size", thighsize, BodyStepC, -1)
			Return()
	...
	+ "Waist.. | W" .. waistsize .. " | H" .. hipssize [gold]
		+ "Hips >"
			hipssize = BodyEditUp(human, "Hips size", hipssize, BodyStepB, 5)
			Return()
		+ "< Hips"
			hipssize = BodyEditDown(human, "Hips size", hipssize, BodyStepB, -2)
			Return()
		+ "RESET Hips | " .. hipssize [gold]
			hipssize = BodyEditSet(human, "Hips size", 0)
			Return()
		+ "Waist >"
			waistsize = BodyEditUp(human, "Waist size", waistsize, BodyStepB, 5)
			Return()
		+ "< Waist"
			waistsize = BodyEditDown(human, "Waist size", waistsize, BodyStepB, -1)
			Return()
	...
	+ if human.m_isMale == false
		+ "Breasts.. | B" .. breastsize .. " | N" .. nipplesize [gold]
			...
			+ "RESET Nipples | " .. nipplesize [gold]
				...
				Return()
			+ "Breasts >"
				breastsize = BodyEditUp(human, "Breasts size", breastsize, BodyStepB, 10)
				Return()
			+ "< Breasts"
				breastsize = BodyEditDown(human, "Breasts size", breastsize, BodyStepB, -2, BreastSafeMin)
				Return()
	...
	+ "Body.. | B" .. bodysize .. " | M ".. musclesize [gold]
		+ "Muscle >"
			musclesize = BodyEditUp(human, "Muscle tone", musclesize, BodyStepB, 1)
			Return()
		+ "< Muscle"
			musclesize = BodyEditDown(human, "Muscle tone", musclesize, BodyStepB, -0.3)
			Return()
		+ "RESET Muscle | " .. musclesize [gold]
			musclesize = BodyEditSet(human, "Muscle tone", 0)
			Return()
		+ "Body >"
			bodysize = BodyEditUp(human, "Body size", bodysize, BodyStepD, 1)
			Return()
		+ "< Body"
			bodysize = BodyEditDown(human, "Body size", bodysize, BodyStepD, -0.9)
			Return()
