...
function OnPenetration(girl, holeName, inVelocity, outVelocity, penetrator)
	...
	local crawlthreshold = 0.1
	local slowthreshold = 0.2
	local normalthreshold = 0.3
	local mediumthreshold = 0.4
	local fasttreshold = 0.6
	local ultratreshold = 0.8

	if inVelocity > 0.01 and inVelocity < crawlthreshold and lastMoanTime > 8 then 
	...
	elseif inVelocity >= slowthreshold and inVelocity < normalthreshold and lastMoanTime > 2 then 
		...
		girl.SayCustom("gen_moan_slow")
		ResetTimer(key)
	elseif inVelocity >= normalthreshold and inVelocity < mediumthreshold and lastMoanTime > 1.6 then 
		girl.SayCustom(girl.Name.."_moan_normal")
		girl.SayCustom("gen_moan_normal")
		ResetTimer(key)
	elseif inVelocity >= mediumthreshold and inVelocity < fasttreshold and lastMoanTime > 1 then 
		girl.SayCustom(girl.Name.."_moan_normal")
		girl.SayCustom("gen_moan_normal")
		ResetTimer(key)
	elseif inVelocity >= fasttreshold and inVelocity < ultratreshold and lastMoanTime > 0.9 then 
