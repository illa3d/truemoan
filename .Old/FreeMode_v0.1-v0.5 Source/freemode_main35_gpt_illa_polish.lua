...
function OnPenetration(girl, holeName, inVelocity, outVelocity, penetrator)
	-- Speed config (speed roughly 0-1, more than that is when weight is > 0)
	local normaltreshold = 0.25
	local fasttreshold = 0.5
	local orgasmtreshold = 0.75
	local nopausetreshold = 0.9
	-- Variables
	local key = "PenetrationMoan_" .. girl.Name .. holeName
	local lastMoanTime = Timer(key)
	local tier = ""
	local baseCooldown = 0
	local killpause = false

	-- Tier selection + boundary detection
	if inVelocity > orgasmtreshold then
		tier = "orgasm"
		baseCooldown = 0.4 -- Orgasm audio files: ~ 0.4sec
		killpause = true -- Orgasm removes pause between moans
	elseif inVelocity > fasttreshold then
		tier = "fast"
		baseCooldown = 0.5 -- Normal audio files: ~ 0.5sec
		if inVelocity > orgasmtreshold * nopausetreshold then
			killpause = true
		end
	elseif inVelocity > normaltreshold then
		tier = "normal"
		baseCooldown = 1.0 -- Normal audio files: ~ 1sec
		if inVelocity > fasttreshold * nopausetreshold then
			killpause = true
		end
	else
		tier = "slow"
		baseCooldown = 2.0 -- Slow audio files: ~ 2sec
		if inVelocity > normaltreshold * nopausetreshold then
			killpause = true
		end
	end
	...
	-- Dynamic breath compression: sustained activity shortens pauses smoothly
	effectiveCooldown = baseCooldown * math.max(0.6, 1 - math.min(lastMoanTime / baseCooldown, 1.5) * 0.3)
	-- Speed induced variation
	if inVelocity > outVelocity then cooldown = cooldown * 0.9 end
	-- Kill pauses near tier end
	if killpause then cooldown = math.min(effectiveCooldown, 0.08) end
	-- Add pause variation
	cooldown = cooldown + math.random() * 0.1
