...
function OnPenetration(girl, holeName, inVelocity, outVelocity, penetrator)
	...
	local slow1, slow2 = 0.1, 0.2
	local normal1, normal2 = 0.3, 0.4
	local fast1, fast2 = 0.5, 0.6
	local orgasm1, orgasm2 = 0.8, 0.9

	-- Ignore micro jitter
	if inVelocity < 0.05 then return end

	-- SLOW
	if inVelocity < slow2 then
		if lastMoanTime > 1.8 then
	...
	-- NORMAL
	elseif inVelocity < normal2 then
		if lastMoanTime > 1.0 then
	...
	-- FAST
	elseif inVelocity < fast2 then
		if lastMoanTime > 0.5 then
