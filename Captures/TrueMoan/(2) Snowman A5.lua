...
local point = MapPoint("map_seventh_heaven_xmas", Pos(-4.787, 0.031, 4.993), Rot(0, 0, 0), overridePos, overrideRot, function()
...
actorA.PlaceAt(Pos(-0.208, 0.116, 0.157), Rot(0, 122.819, 0), point)
actorB.PlaceAt(Pos(0.209, -0.116, -0.156), Rot(0, 307.621, 0), point)
...
actorB.Body("Penis size", 0.03239369, "Penis length", 0.2492741, "Ass size", -1, "Hips size", -0.2, "Thigh size", -0.2, "Calf size", -0.1, "Nipples size", 0.3180024, "Upper arms size", -0.09999999, "Forearms size", -0.1, "Muscle tone", 0.3, "Fluid speed", 1, "Fluid spread", 1.420524, "Fluid amount", -0.5, "Fluid force", 0.1055639)
...
PoseCam(Pos(0.015, 2.85, 0.112), Pos(-0.022, 0.978, 0.137), point)
...
NewClip().Layer(MotionLayer.PlayerPose).Fade(fadeTime).Actors(actorA).WrapMode(MotionWrap.Loop).Data(
	...
	HeadRot(275.014, 168.961, 191.065),
	EyeL(CameraPos()),
	EyeR(CameraPos()),
...
NewClip().Layer(MotionLayer.PlayerPose).Fade(fadeTime).Actors(actorB, actorA).WrapMode(MotionWrap.Loop).Data(
	Penis(TargetActor.A, "Vagina", TargetActor.Null, "", 0, 0, 0, 0, true, 0.7, 0.3, 0.9, 1.317, false, 0.25, 0.65, 1.25, 1.652, 1.2),
	...
	EyeL(CameraPos()),
	EyeR(CameraPos()),
	FootL(WSurface(TPos(-0.481, 0.186, 0.175), 0, -1, 0.004)),
