...
function OnPenetration(girl, holeName, inVelocity, outVelocity, penetrator)
	...
	local nearTierEnd = false
	-- Tier selection + boundary detection
	if inVelocity < 0.30 then
		tier = "slow"
		baseCooldown = 2.0
		if inVelocity > 0.26 then
			nearTierEnd = true
		end
	elseif inVelocity < 0.50 then
		tier = "normal"
		baseCooldown = 1.0
		if inVelocity > 0.46 then
			nearTierEnd = true
		end
	elseif inVelocity < 0.80 then
		tier = "fast"
		baseCooldown = 0.5
		if inVelocity > 0.74 then
			nearTierEnd = true
		end
	else
		tier = "orgasm"
		baseCooldown = 0.4
		nearTierEnd = true -- orgasm always chains
	end
	-- Organic timing
	local effectiveCooldown = baseCooldown
	-- Dynamic compression while active
	...
	if nearTierEnd then
		effectiveCooldown = math.min(effectiveCooldown, 0.08)
	end
	-- Micro randomness
	effectiveCooldown = effectiveCooldown + math.random() * 0.08
	-- Play
