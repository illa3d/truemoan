	...
	+ "SEX.."[gold]
		...
		+ "Thrust.. | " .. human.Penis.Interaction.m_autoSpeed [if human.Penis.Hole ~= nil] [gold]
			+ "5 Max"
				...
				Return()
			+ "4 Fast"
				SetInteractionSpeed(human.Penis.Interaction, 0.8)	
				Return()
			+ "3 Normal"
		...
		+ "Suck.. | " .. human.Mouth.Fucker.Penis.Interaction.m_autoSpeed [if human.Mouth.Fucker ~= nil] [gold]
			+ "5 Max"
				...
				Return()
			+ "4 Fast"
				SetInteractionSpeed(human.Mouth.Fucker.Penis.Interaction, 0.8)	
				Return()
			+ "3 Normal"
				SetInteractionSpeed(human.Mouth.Fucker.Penis.Interaction, 0.4)	
				Return()
			+ "2 Medium"
...
function SetInteractionSpeed(interaction, speed)
	speed = math.max(sexspeedmin, math.min(speed, sexspeedmax))
...
function SetInteractionSpeedStep(interaction, increase)
	...
	if speed > 1 then multiplier = multiplier / 2
	elseif speed > 0.5 then multiplier = multiplier / 1.5
	elseif speed < 0.2 then multiplier = multiplier * 2
	elseif speed < 0.1 then multiplier = multiplier * 4 end
	elseif speed < 0.05 then multiplier = multiplier * 8 end

	if increase then speed = speed * (1 + multiplier)
	else speed = speed * (1 - multiplier) end
