...
function OnPenetration(girl, holeName, inVelocity, outVelocity, penetrator)
	-- Speed config by sex speed
	local normaltreshold = 0.2
	local fasttreshold = 0.6
	local orgasmtreshold = 0.9
	...
	if inVelocity > orgasmtreshold then
		tier = "orgasm"
		pauseMax = 0.4 -- Orgasm audio files: ~0.4s + pause
		tierMin = orgasmtreshold
		tierMax = 1.0
	elseif inVelocity > fasttreshold then
		tier = "fast"
		pauseMax = 1 -- Orgasm audio files: ~0.5s + pause
		tierMin = fasttreshold
		tierMax = orgasmtreshold
	elseif inVelocity > normaltreshold then
		tier = "normal"
		pauseMax = 1.3 -- Orgasm audio files: ~1s + pause
		tierMin = normaltreshold
		tierMax = fasttreshold
	else
		tier = "slow"
		pauseMax = 20.0 -- VERY long pauses when not moving
		tierMin = 0.0
		tierMax = normaltreshold
	end

	-- ORGANIC PAUSES: Speed-based pause scaling: long to zero near tier end
	-- End deadzone (no pauses at the end)
	tierMax = tierMax * 0.9
	-- Normalize speed within current tier (0..1)
	...
	local breathCompression =
	math.max(0.6, 1 - math.min(lastMoanTime / math.max(pauseMax, 0.01), 1.5) * 0.3)
	cooldown = cooldown * breathCompression
	-- Direction variation (inward motion tightens rhythm)
