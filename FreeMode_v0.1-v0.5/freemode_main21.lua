...
label TalkMenu(human, hitTri)
	...
	+ "BODY.."[gold]
		...
		+ if human.m_isMale == false
			+ "Breasts.. | B" .. breastssize .. " | N" .. nippsize [gold]
				+ "Nipples >"
					nippsize = nippsize + sizestep03
					if (nippsize > 4)
						nippsize = 4
					human.Body("Nipples size", nippsize)
					Return()
				+ "< Nipples"
					nippsize = nippsize - sizestep03
					if (nippsize < -1)
		...
		+ "Body.. | B" .. bodysize .. " | M ".. musclesize [gold]
			...
			+ "DONE"
		+ "Apply All"
			human.Body("Neck size", calfsize)
			human.Body("Forearms size", calfsize)
			human.Body("Upper arms size", thighsize)
			human.Body("Calf size", calfsize)
			human.Body("Thigh size", thighsize)
			human.Body("Hips size", calfsize)
			human.Body("Waist size", thighsize)
			human.Body("Ass size", asssize)
			human.Body("Nipples size", breastssize)
			human.Body("Breasts size", breastssize)
			human.Body("Penis length", penislength)
			human.Body("Penis size", penissize)
			human.Body("Muscle tone", bodysize)
			human.Body("Body size", bodysize)
			human.Penis.m_penisSkinOut = 0
			Return()
		+ "< BACK"
	...
	+ "SEX.."[gold]
		...
		+ if human.Penis.Hole ~= nil
			+ "Speed.. | " .. human.Penis.Interaction.m_autoSpeed [gold]
				+ "4 Fastest"
		...
		+ "Auto BJ" [if human.Mouth.Fucker ~= nil and human.Mouth.Fucker.Penis.Interaction.AutoActive == false and activeAutoBJ == false]
			...
			activeAutoBJ = true
			Return()
		+ "Stop AutoBJ" [if human.Mouth.Fucker ~= nil and activeAutoBJ == true]
			--human.Mouth.Fucker.Penis.Interaction.AutoActive = false
			StopAutoBJ(human)
			activeAutoBJ = false
			Return()
