...
function OnPenetration(girl, holeName, inVelocity, outVelocity, penetrator)
	if inVelocity > outVelocity and game.FluidReaction == true then

		-- Speed config by sex speed
		local orgasmtreshold = 0.9
		local fastertreshold = 0.6
		local fasttreshold = 0.3
		local normaltreshold = 0.1
		...
		-- Deadzone - no pause before tier end
		-- tierMax = tierMax * 0.8
		...
		--local breathCompression = math.max(0.6, 1 - math.min(lastMoanTime / math.max(pauseMax, 0.01), 1.5) * 0.3)
		--cooldown = cooldown * breathCompression
		-- Small randomness to avoid mechanical timing
		cooldown = cooldown + math.random() * 0.03
