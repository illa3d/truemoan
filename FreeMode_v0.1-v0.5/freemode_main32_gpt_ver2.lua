...
function OnPenetration(girl, holeName, inVelocity, outVelocity, penetrator)
	...
	-- 🔑 Tier jump smoothing:
	-- If velocity is clearly high, allow immediate continuation
	if inVelocity > outVelocity and lastMoanTime > 0.05 then
		cooldown = math.min(cooldown, 0.1)
	end

	-- Small randomness prevents metronome effect
	cooldown = cooldown + math.random() * 0.12
