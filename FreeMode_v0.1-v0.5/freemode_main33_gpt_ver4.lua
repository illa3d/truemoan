...
function OnPenetration(girl, holeName, inVelocity, outVelocity, penetrator)
	...
	-- Tier selection (clean, single pass)
	...
	-- Organic pause shaping
	local effectiveCooldown = baseCooldown

	-- Gradual compression while staying active
	...
	-- Escalation bias (chain upward tiers)
	if inVelocity > outVelocity then
		effectiveCooldown = effectiveCooldown * 0.9
	end

	-- Micro-randomness to avoid metronome feel
	effectiveCooldown = effectiveCooldown + math.random() * 0.1
