...
function OnPenetration(girl, holeName, inVelocity, outVelocity, penetrator)
	...
	-- Ignore tiny jitter
	if inVelocity < 0.05 then return end

	local tier = ""
	local baseCooldown = 0

	-- Tier selection (simple, clean)
	if inVelocity < 0.30 then
		tier = "slow"
		baseCooldown = 2.2
		if inVelocity > 0.20 then
			baseCooldown = 1.6
		elseif inVelocity > 0.12 then
			baseCooldown = 1.9
		end

	elseif inVelocity < 0.50 then
		tier = "normal"
		baseCooldown = 1.2
		if inVelocity > 0.40 then
			baseCooldown = 0.9
		end

	elseif inVelocity < 0.80 then
		tier = "fast"
		baseCooldown = 0.6
		if inVelocity > 0.65 then
			baseCooldown = 0.45
		end
	...
	-- -------- Organic timing logic --------
	local effectiveCooldown = baseCooldown

	-- Gradual pause shortening while staying active in the tier
	if lastMoanTime > baseCooldown then
		effectiveCooldown = baseCooldown * 0.6
	elseif lastMoanTime > baseCooldown * 0.75 then
		effectiveCooldown = baseCooldown * 0.75
	elseif lastMoanTime > baseCooldown * 0.5 then
		effectiveCooldown = baseCooldown * 0.85
	end

	-- Favor continuous escalation (no pause when ramping up)
	if inVelocity > outVelocity then
		effectiveCooldown = effectiveCooldown * 0.9
	end

	-- Small randomness to avoid robotic rhythm
	effectiveCooldown = effectiveCooldown + math.random() * 0.12

	-- -------- Play --------
	if lastMoanTime > effectiveCooldown then
