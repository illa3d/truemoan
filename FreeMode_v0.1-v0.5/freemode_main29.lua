...
label TalkMenu(human, hitTri)
	...
	+ "SEX.."[gold]
		...
		+ "Auto Sucking" [if human.Mouth.Fucker ~= nil and human.Mouth.Fucker.Penis.Interaction.AutoActive == false and activeAutoBJ == false]
			...
			activeAutoBJ = true
			Return()
		+ "Thrust Speed.. | " .. human.Penis.Interaction.m_autoSpeed [if human.Penis.Hole ~= nil] [gold]
			+ "4 Fastest"
				human.Penis.Interaction.m_autoSpeed = 2
				Return()
			+ "3 Fast"
				human.Penis.Interaction.m_autoSpeed = 0.8
				Return()
			+ "2 Medium"
				human.Penis.Interaction.m_autoSpeed = 0.5
				Return()
			+ "1 Slow"
				human.Penis.Interaction.m_autoSpeed = 0.2
				Return()
			+ "Faster >"
			...
			+ "RESET | " .. human.Penis.Interaction.m_autoSpeed [gold]
				human.Penis.Interaction.m_autoSpeed = 0.0001
				Return()
			+ "< BACK"
				Return(2)
			+ "DONE"
		+ "Suck Speed.. | " .. human.Mouth.Fucker.Penis.Interaction.m_autoSpeed [if human.Mouth.Fucker ~= nil] [gold]
			+ "4 Fastest"
				human.Mouth.Fucker.Penis.Interaction.m_autoSpeed = 2
				Return()
			+ "3 Fast"
				human.Mouth.Fucker.Penis.Interaction.m_autoSpeed = 0.8
				Return()
			+ "2 Medium"
				human.Mouth.Fucker.Penis.Interaction.m_autoSpeed = 0.5
				Return()
			+ "1 Slow"
				human.Mouth.Fucker.Penis.Interaction.m_autoSpeed = 0.1
				Return()
			+ "Faster >"
				human.Mouth.Fucker.Penis.Interaction.m_autoSpeed = human.Mouth.Fucker.Penis.Interaction.m_autoSpeed * 1.1
				Return()
			+ "< Slower"
				human.Mouth.Fucker.Penis.Interaction.m_autoSpeed = human.Mouth.Fucker.Penis.Interaction.m_autoSpeed * 0.9
				Return()
			+ "RESET | " .. human.Mouth.Fucker.Penis.Interaction.m_autoSpeed [gold]
				human.Mouth.Fucker.Penis.Interaction.m_autoSpeed = 0.0001
				Return()
			+ "< BACK"
				Return(2)
			+ "DONE"
...
function OnFluidHit(hitActor, bodyArea, shootActor)
	...
	if bodyArea == "L_Eye" and lastHitTime > 1 then 
		if game.FluidReaction == true then
			hitActor.SayCustom(hitActor.Name.."_moan_fast")
	...
	elseif bodyArea == "Lips" and lastHitTime > 1 then 
		if game.FluidReaction == true then
			hitActor.SayCustom(hitActor.Name.."_moan_slow")
		...
		if lastHitTime > 10 and game.FluidReaction == true then
			hitActor.SayCustom(hitActor.Name.."_moan_normal")
			hitActor.SayCustom("gen_moan_normal")
			ResetTimer(timerKey)
		end
	end
end
...
function OnPenetration(girl, holeName, inVelocity, outVelocity, penetrator)
	...
	local slowthreshold1 = 0.1
	local slowthreshold2 = 0.2
	local normalthreshold1 = 0.3
	local normalthreshold2 = 0.4
	local fasttreshold1 = 0.5
	local fasttreshold2 = 0.6
	local orgasmtreshold1 = 0.8
	local orgasmtreshold2 = 0.9

	if inVelocity > 0.01 and inVelocity < slowthreshold1 and lastMoanTime > 8 then 
		girl.SayCustom(girl.Name.."_moan_slow")
		girl.SayCustom("gen_moan_slow")
		ResetTimer(key)
	elseif inVelocity >= slowthreshold1 and inVelocity < slowthreshold2 and lastMoanTime > 4 then 
		girl.SayCustom(girl.Name.."_moan_slow")
		girl.SayCustom("gen_moan_slow")
		ResetTimer(key)
	elseif inVelocity >= slowthreshold2 and inVelocity < normalthreshold1 and lastMoanTime > 0.001 then 
		girl.SayCustom(girl.Name.."_moan_slow")
		girl.SayCustom("gen_moan_slow")
		ResetTimer(key)
	elseif inVelocity >= normalthreshold1 and inVelocity < normalthreshold2 and lastMoanTime > 1 then 
		girl.SayCustom(girl.Name.."_moan_normal")
		girl.SayCustom("gen_moan_normal")
		ResetTimer(key)
	elseif inVelocity >= normalthreshold2 and inVelocity < fasttreshold1 and lastMoanTime > 0.001 then 
		girl.SayCustom(girl.Name.."_moan_normal")
		girl.SayCustom("gen_moan_normal")
		ResetTimer(key)
	elseif inVelocity >= fasttreshold1 and inVelocity < fasttreshold2 and lastMoanTime > 1 then 
		girl.SayCustom(girl.Name.."_moan_fast")
		girl.SayCustom("gen_moan_fast")
		ResetTimer(key)
	elseif inVelocity >= fasttreshold2 and inVelocity < orgasmtreshold1 and lastMoanTime > 0.001 then 
		girl.SayCustom(girl.Name.."_moan_fast")
		girl.SayCustom("gen_moan_fast")
		ResetTimer(key)
	elseif inVelocity >= orgasmtreshold1 and inVelocity < orgasmtreshold2 and lastMoanTime > 0.4 then 
		girl.SayCustom(girl.Name.."_moan_orgasm")
		girl.SayCustom("_moan_orgasm")
		ResetTimer(key)
	elseif inVelocity >= orgasmtreshold2 and lastMoanTime > 0.001 then 
		girl.SayCustom(girl.Name.."_moan_orgasm")
		girl.SayCustom("gen_moan_orgasm")
		ResetTimer(key)
	end
end
